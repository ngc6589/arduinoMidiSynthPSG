...
public:
    inline void add(const MidiNote& inNote, byte mch);
    inline void remove(byte inPitch);
    inline void remove(byte inPitch, byte mch);
...
public:
    ...
    inline bool getIdx(byte& mIdx, byte outPitch) const;
    inline bool getIdx(byte& mIdx, byte outPitch, byte inChannel) const;
 ...
 */
template<byte Size>
inline void MidiNoteList<Size>::add(const MidiNote& inNote, byte mch)
{
    if (mHead == 0)
    {
        mArray[0].note   = inNote;
        mArray[0].PSGChannel = 0;
        mArray[0].MIDIChannel = 0;
    ...
    else
    {
        ...
        newTail->active = true;
        newTail->note = inNote;
        newTail->PSGChannel = mSize;
        newTail->MIDIChannel = mch;
...
template<byte Size>
inline void MidiNoteList<Size>::remove(byte inPitch, byte inchannel)
{
    if (mTail != 0)
    {
        for (Cell* it = mTail; it != 0; it = it->prev)
        {
            if ((it->note.pitch == inPitch) && (it->MIDIChannel == inchannel))
            {
                Cell* const prev = it->prev;
                Cell* const next = it->next;
...
template<byte Size>
inline bool MidiNoteList<Size>::getIdx(byte& mIdx, byte outPitch) const
{
    bool  ret = false;
...
template<byte Size>
inline bool MidiNoteList<Size>::getIdx(byte& mIdx, byte outPitch, byte inChannel) const
{
    bool  ret = false;
    ...
    for (byte i = 0; i < mSize; ++i)
    {
        if ((it->note.pitch == outPitch) && (it->MIDIChannel == inChannel))
        {
            mIdx = it->PSGChannel;
            ret = true;
            break;
